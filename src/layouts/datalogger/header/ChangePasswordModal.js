...
export default function ChangePasswordModal({ openResetPasswordModal, setOpenResetPasswordModal }) {
    ...
    const validationSchema = Yup.object().shape({
        ...
        password: Yup.string()
            .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, "Password must contain at least 8 characters, one uppercase, one lowercase, one number and one special case character")
    ...
    const handleChangePassword = (data) => {
        ...
        setTimeout(async () => {
            try {
                const response = await axiosPrivate.post(Constants.API_URL.USERS.CHANGE_PASSWORD, body);
    ...
    return (
        ...
        >
            <FormInput id="changePasswordForm" initialValues={initialChangePassword} onSubmit={handleChangePassword} validationSchema={validationSchema}>
