...
function Components() {
    const { isExpand, handleOnExpand, inverterList, setInverterList, weatherStationList, setWeatherStationList, productionMeterList, setProductionMeterList } = useComponents();
        ...
        <div className={styles.components}>
            ...
            {isExpand &&
                <div className={styles.body} >
                    <ExpandableList
                        name="PV System Inverter (5)"
                        list={inverterList}
                        onChange={list => setInverterList(list)}
                    />
                    
                    <ExpandableList
                        name="Weather Station (2)"
                        list={weatherStationList}
                        onChange={list => setWeatherStationList(list)}
                    />
