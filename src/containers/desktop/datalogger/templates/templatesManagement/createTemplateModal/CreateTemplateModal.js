...
function CreateTemplateModal({ isOpen, close, template, duplicate }) {
  const {
    handleOnSubmit,
    initialValues,
    validationSchema,
    ...
    groups,
    onClose,
  } = useSubmitTemplate(close, duplicate);
  ...
  return (
    ...
    >
        ...
        <div className="row mb-3">
          <div className="col-11">
            <FormInput.Select
              ...
              option={groups}
              required={true}
              placeholder={`Select group of new template`}
              groupOption={true}
              isSearchable={true}
            />
          </div>
      ...
      <Button variant="dark" type="submit" formId="template-form">
        <Button.Text text={`Yes, Create new template`} />
      </Button>
