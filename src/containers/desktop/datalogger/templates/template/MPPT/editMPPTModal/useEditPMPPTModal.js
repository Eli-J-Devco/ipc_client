...
function useEditMPPTModal(data, close, setPoint, setCurrentData) {
    ...
    const [selectedUnit, setSelectedUnit] = useState({ value: data?.type_units?.id, label: data?.type_units?.namekey });
    const [selectedDataType, setSelectedDataType] = useState({ value: data?.type_datatype?.id, label: data?.type_datatype?.namekey });
    const [selectedByteOrder, setSelectedByteOrder] = useState({ value: data?.type_byteorder?.id, label: data?.type_byteorder?.namekey });
    const [selectedPointListType, setSelectedPointListType] = useState({ value: data?.type_point_list?.id, label: data?.type_point_list?.namekey });
    const [selectedControlGroup, setSelectedControlGroup] = useState({ value: data?.type_control?.id, label: data?.type_control?.namekey });
    ...
    const initialValues = {
        ...
        data_type: { value: data?.type_datatype?.id, label: data?.type_datatype?.namekey },
        byte_order: { value: data?.type_byteorder?.id, label: data?.type_byteorder?.namekey },
        type_point_list: { value: data?.type_point_list?.id, label: data?.type_point_list?.namekey },
        type_control: { value: data?.type_control?.id, label: data?.type_control?.namekey },
    ...
    const onSubmit = (values) => {
        ...
        let point = {
            ...
            type_units: selectedUnit.value ? { id: selectedUnit.value, namekey: selectedUnit.label } : null,
            type_datatype: selectedDataType.value ? { id: selectedDataType.value, namekey: selectedDataType.label } : null,
            type_byteorder: selectedByteOrder.value ? { id: selectedByteOrder.value, namekey: selectedByteOrder.label } : null,
            type_point_list: selectedPointListType.value ? { id: selectedPointListType.value, namekey: selectedPointListType.label } : null,
            type_control: selectedControlGroup.value ? { id: selectedControlGroup.value, namekey: selectedControlGroup.label } : null,
        };
        var output = document.getElementById("progress");
        output.innerHTML = "<div><img src='/loading.gif' /></div>";
        setTimeout(async () => {
            try {
                const response = await axiosPrivate.post(Constants.API_URL.TEMPLATE.POINT.UPDATE, {
                    ...point
                },
                    {
                        headers: {
