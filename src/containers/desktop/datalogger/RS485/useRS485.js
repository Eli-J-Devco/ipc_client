...
export default function useRS485() {
  ...
  useEffect(() => {
    ...
    Object.entries(rs485Config).forEach(([key, value]) => {
      ...
      let selected = value.filter(
        (item) => item.id === existedRS485[index[[key]]]
      );
  ...
  const fetchRS485 = async (id) => {
    try {
      const response = await axiosPrivate.post(Constants.API_URL.RS485.GET, {
        id: id,
      });
      ...
      loginService.handleMissingInfo(error, "RS485 error") &&
