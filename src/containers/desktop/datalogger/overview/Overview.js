...
function Overview() {
    ...
    useEffect(() => {
        if (!_.isEmpty(cpuData) && !_.isEqual(cpuData, curInformationCPU)) {
            ...
            let CPUPercentage = !_.isEmpty(cpuData.CPUInfo.TotalCPUUsage) ? cpuData.CPUInfo.TotalCPUUsage.replace("%", "") : 0;
            ...
            // RAM
            let RAMInfo = !_.isEmpty(cpuData.MemoryInformation) && cpuData.MemoryInformation;
            let RAMPercentage = !_.isEmpty(RAMInfo.Percentage) ? RAMInfo.Percentage.replace("%", "") : "";
            ...
            //SWAP
            let SWAPInfo = !_.isEmpty(cpuData.MemoryInformation.SWAP) && cpuData.MemoryInformation.SWAP;
            let SWAPPercentage = !_.isEmpty(SWAPInfo.Percentage) ? SWAPInfo.Percentage.replace("%", "") : 0;
            ...
            //CPU
            let totalCPU = !_.isEmpty(cpuData.CPUInfo) ? cpuData.CPUInfo.Totalcores : 0;
            let CPUseries = [{
                name: 'Speed',
            ...
            //Disk
            let DISKInfo = !_.isEmpty(cpuData.DiskInformation) && cpuData.DiskInformation;
            let DiskPercentage = !_.isEmpty(DISKInfo.Percentage) ? DISKInfo.Percentage.replace("%", "") : 0;
            let DISKseries = [{
                name: 'Speed',
            ...
            let timeOfRunning = "";
            if (!_.isEmpty(cpuData.BootTime) && !_.isEmpty(cpuData.BootTime.BootTime)) {
                ...
                if (diffSeconds <= 5) {
                    timeOfRunning = "less than a minute";
                }
                if (diffMinutes > 0) {
                    timeOfRunning = diffMinutes === 1 ? `a minute` : `${diffMinutes} minutes`;
                }
                if (diffHours > 0) {
                    timeOfRunning = diffHours === 1 ? `an hour` : `${diffHours} hours`;
                }
                if (diffDays > 0) {
                    timeOfRunning = diffDays === 1 ? `a day` : `${diffDays} days`;
                }
            }
            setTimeout(() => {
                ...
                setTimeOfRunning(timeOfRunning);
            }, 100);
        }
    }, [cpuData]);
    ...
    return (
        <div className="main">
            <div className={styles.overview}>
                    ...
                    <Modal
                        ...
                        footer={
                            ...
                            <div className="d-flex w-100 flex-row justify-content-center">
                                <Button className="me-1" variant="white" onClick={() => setIsOpenRebootCFModal(false)}>Cancel</Button>
                                <Button className="ms-1" variant="dark" onClick={() => setIsReboot(true)}>Reboot</Button>
                            </div>
                        }
                        onClose={() => {
                            !isReboot &&
                ...
                {
                    !_.isEmpty(CPU) && !_.isEmpty(RAM) && !_.isEmpty(DISK) && !_.isEmpty(SWAP) &&
                    <div className={styles.outlet}>
                        ...
                        <div className={styles.system_info}>
                            ...
                            <span>{`System: ${!_.isEmpty(curInformationCPU.SystemInformation) ? `${curInformationCPU.SystemInformation.System} ${curInformationCPU.SystemInformation.Version}` : ""}`}</span>
                            <span className={styles.uptime}>Time of running: {!_.isEmpty(timeOfRunning) && timeOfRunning}</span>
                            {
                                !_.isEmpty(cpuData) &&
                                <Button className="ms-5" variant="dark" onClick={() => setIsOpenRebootCFModal(!isOpenRebootCFModal)}>
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="col-md-3">
                                            <div className={styles.item}>
                                                ...
                                                <p>{!_.isEmpty(RAMInfo.Used) && !_.isEmpty(RAMInfo.Total) ? `${RAMInfo.Used} / ${RAMInfo.Total}` : ""}</p>
                                            </div>
                                        </div>
                                        <div className="col-md-3">
                                            <div className={styles.item}>
                                                ...
                                                <p>{!_.isEmpty(DISKInfo.Used) && !_.isEmpty(DISKInfo.TotalSize) ? `${DISKInfo.Used} / ${DISKInfo.TotalSize}` : ""}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="col-md-3">
                                            <div className={styles.item}>
                                                ...
                                                <p>{!_.isEmpty(SWAPInfo.Used) && !_.isEmpty(SWAPInfo.Total) ? `${SWAPInfo.Used} / ${SWAPInfo.Total}` : ""}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
