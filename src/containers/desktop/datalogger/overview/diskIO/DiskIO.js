...
function DiskIO() {
  ...
  useEffect(() => {
    if (
      _.isEmpty(cpuData) ||
      _.isEmpty(cpuData.DiskIO) ||
      _.isEqual(cpuData, curItem)
    )
      return;
    let DiskIO = {};
    DiskIO.ReadBytes = !_.isEmpty(cpuData.DiskIO.ReadBytes)
      ? cpuData.DiskIO.ReadBytes
      : "";
    DiskIO.SpeedRead = !_.isEmpty(cpuData.DiskIO.SpeedRead)
      ? cpuData.DiskIO.SpeedRead
      : "";
    DiskIO.SpeedWrite = !_.isEmpty(cpuData.DiskIO.SpeedWrite)
      ? cpuData.DiskIO.SpeedWrite
      : "";
    DiskIO.WriteBytes = !_.isEmpty(cpuData.DiskIO.WriteBytes)
      ? cpuData.DiskIO.WriteBytes
      : "";

    let SpeedReadPercentage = !_.isEmpty(cpuData.DiskIO.SpeedRead)
      ? cpuData.DiskIO.SpeedRead.split(" ")
      : 0;
    let SpeedWritePercentage = !_.isEmpty(cpuData.DiskIO.SpeedWrite)
      ? cpuData.DiskIO.SpeedWrite.split(" ")
      : 0;
    let time = Math.floor(cpuData.Time / 1000) * 1000;

    const chart = chartRef.current.chart;
    const pointSpeedRead = [
      new Date(time).getTime(),
      parseFloat(SpeedReadPercentage[0]),
    ];
    const pointSpeedWrite = [
      new Date(time).getTime(),
      parseFloat(SpeedWritePercentage[0]),
    ];
    ...
    <div className={styles.group_disk_io}>
      <div className="row">
        <div className="col-md-3">
          <p>
            <span className={styles.ico_up}></span>Read
          </p>
          <p>{DiskIO.SpeedRead}</p>
        </div>
        <div className="col-md-3">
          <p>
            <span className={styles.ico_down}></span>Write
          </p>
